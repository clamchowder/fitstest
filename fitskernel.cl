...
// utterly unoptimized
__kernel void calculate_potential(__global double *data, long x_len, long y_len, __global double *result) {
  ...
  __local double scratchdata[256];
  int localid = get_local_id(0);
  long x_pos = get_global_id(0) % x_len;
  long y_pos = get_global_id(0) / x_len;
